...
const Scripts: React.FC = () => {
    ...
    const handleQuickDelete = async (script: Script, e: React.MouseEvent) => {
        e.stopPropagation();
        if (!confirm(`Möchtest du das Script "${script.name}" wirklich löschen?`)) return;
        ...
        <div className="px-4 py-6 sm:px-0">
            <div className="mb-8 flex justify-between items-center">
                <div>
                    ...
                    <p className="mt-2 text-secondary">Verwalte deine Deployment-Scripts und Automatisierungen</p>
                </div>
                <button
                ...
                {scripts.length === 0 ? (
                  ...
                  <span className="flex items-center">
                    <FileText className="w-3 h-3 mr-1" />
                      {script.variables?.length || 0} Variable{(script.variables?.length || 0) !== 1 ? 'n' : ''}
                  </span>
                                </div>
                                <span>
                  {format(new Date(script.updated_at), 'dd.MM.yyyy', { locale: de })}
                </span>
                            </div>
