...
export class AWSClient {
  ...
  async getECSServices(clusterArn?: string): Promise<ECSService[]> {
    try {
      const ecs = new ECSClient(this.getClientConfig())
      const clusters = clusterArn ? [clusterArn] : await this.getECSClusters()
      const clusterArns = clusterArn ? [clusterArn] : clusters.map(c => c.clusterArn)
  ...
  async getECSTasks(clusterArn?: string): Promise<ECSTask[]> {
    try {
      const ecs = new ECSClient(this.getClientConfig())
      const clusters = clusterArn ? [{ clusterArn }] : await this.getECSClusters()
      const clusterArns = clusters.map(c => c.clusterArn)
