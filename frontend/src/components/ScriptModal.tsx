...
const ScriptModal: React.FC<ScriptModalProps> = ({
    ...
    useEffect(() => {
        if (script) {
            ...
            setVariables(script.variables.map(v => ({
                name: v.name,
    ...
    const handleSave = async () => {
        ...
        const validVariables = variables.filter(variable => {
            const isValid = variable.name && variable.name.trim() &&
                variable.placeholder && variable.placeholder.trim();
            return isValid;
        });
        setLoading(true);
        try {
    ...
    const handleDelete = async () => {
        if (!script || !confirm(`Möchtest du das Script "${script.name}" wirklich löschen?`)) return;
    ...
    const handleUpdateVariable = (index: number, field: keyof Omit<ScriptVariable, 'id'>, value: any) => {
            ...
            <div className="space-y-6">
                ...
                <div>
                    ...
                    <p className="text-xs text-muted mt-1">
                        Tipp: Verwende <code className="bg-gray-600 px-1 rounded">$J{`{VARIABLE_NAME}`}</code> für Variablen (GitLab-kompatibel)
                    </p>
                </div>
                                    ...
                                    <div className="flex items-center space-x-4">
                                        <label className="flex items-center space-x-2">
                                            <input
                                                ...
                                                className="rounded"
                                            />
                                            <span className="text-xs text-secondary">Erforderlich</span>
                                        </label>
                                        <button
