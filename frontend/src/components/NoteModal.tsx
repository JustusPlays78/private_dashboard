...
const NoteModal: React.FC<NoteModalProps> = ({
    ...
    return (
        <Modal isOpen={isOpen} onClose={handleCancelEdit} title={getModalTitle()} size="3xl">
            <div className="space-y-6">
                ...
                {!isCreateMode && note && (
                    <div className="text-xs text-muted border-t border-accent pt-4">
                        <div className="flex justify-between">
                            <div>
                                <p>Erstellt: {format(new Date(note.created_at), 'dd.MM.yyyy HH:mm', { locale: de })}</p>
                                <p>Zuletzt bearbeitet: {format(new Date(note.updated_at), 'dd.MM.yyyy HH:mm', { locale: de })}</p>
                            </div>
                            <div className="text-right" style={{ color: 'var(--text-placeholder)' }}>
                ...
                {/* Actions */}
                <div className={`flex ${isEditing && !isCreateMode ? 'justify-between' : 'justify-end'} border-t border-accent pt-4`}>
