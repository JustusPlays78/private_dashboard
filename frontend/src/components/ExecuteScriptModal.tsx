...
const ExecuteScriptModal: React.FC<ExecuteScriptModalProps> = ({
  ...
  const handleExportVariables = () => {
    const exportData = script?.variables.reduce(
      (acc, variable) => {
  ...
  const validateVariables = (): string[] => {
    ...
    script?.variables.forEach((variable) => {
      if (
        variable.required &&
        (!variables[variable.name] || variables[variable.name].trim() === '')
      ) {
          ...
          <>
            ...
            <div className="card p-4 border border-accent">
              ...
              <p className="text-xs text-muted mt-2">
                {script.variables.length} Variable
                {script.variables.length !== 1 ? 'n' : ''} erforderlich
              </p>
            </div>
            ...
            <div className="card p-4 border border-accent">
              ...
              <p className="text-xs text-muted mt-2">
                JSON: <code>{`{"VARIABLE_NAME": "value"}`}</code> | Text:{' '}
                <code>VARIABLE_NAME=value</code> | Script:{' '}
            ...
            <div>
              ...
              <div className="space-y-4 max-h-96 overflow-y-auto">
                {script.variables.map((variable) => (
                  <div
                    key={variable.id}
                    className="card p-4 border border-accent"
                  >
            ...
            <div className="space-y-4">
              ...
              <div className="card p-4 border border-accent">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                  {Object.entries(result?.variables_used || {}).map(
                    ([key, value]) => (
                      <div key={key} className="flex">
                        <span className="font-mono text-muted w-32 truncate">
                          {key}:
                        </span>
