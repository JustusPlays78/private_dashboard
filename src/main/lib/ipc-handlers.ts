...
import * as portableTools from './portable-tools'
...
export function setupIpcHandlers(getMainWindow: () => BrowserWindow | null) {
  ...
  ipcMain.handle('auth:changePassword', async (_, currentPassword: string, newPassword: string) => {
    try {
      const db = getDatabase();
      // Verify current password by trying to unlock (if locked) or by checking hash
  ...
  // ===== Portable Tools Handlers =====

  ipcMain.handle('tools:getStatus', async () => {
    updateActivity()
    try {
      const status = await portableTools.getToolsStatus()
  ...
  ipcMain.handle('tools:download', async (_, toolName: string) => {
    updateActivity()
    try {
      const result = await portableTools.downloadTool(toolName, (status, percent) => {
  ...
  ipcMain.handle('tools:isInstalled', async (_, toolName: string) => {
    updateActivity()
    try {
      const status = await portableTools.isToolInstalled(toolName)
