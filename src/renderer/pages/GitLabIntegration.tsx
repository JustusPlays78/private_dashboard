...
import { GitBranch, Download, RefreshCw, Search, FolderGit, Check, AlertCircle, Folder, Trash2, Key, ChevronDown, Server, Link2, Terminal, Loader2 } from 'lucide-react';
...
export default function GitLabIntegration() {
  ...
  const [isDownloadingGit, setIsDownloadingGit] = useState(false);
  const [gitDownloadError, setGitDownloadError] = useState<string | null>(null);

  // Use preloaded state for Git and GitLab config
  useEffect(() => {
    if (appState.git.checked) {
      setGitInstalled(appState.git.installed);
      if (!appState.git.installed) {
        setError('Git is not installed. Please install Git to use GitLab integration.');
      }
    } else {
      // Fallback: check if not preloaded
      checkGitInstallation();
    }

    if (appState.gitlabConfig.checked && appState.gitlabConfig.configured) {
      setConfigured(true);
      if (appState.gitlabConfig.baseUrl) {
        setBaseURL(appState.gitlabConfig.baseUrl);
      }
    } else if (!appState.gitlabConfig.checked) {
      // Fallback: load config if not preloaded
      loadSavedConfig();
    }
  }, [appState.git, appState.gitlabConfig]);
  ...
  const downloadGit = async () => {
    setIsDownloadingGit(true);
    setGitDownloadError(null);
    
    // Show progress toast
    toast.showProgress('git-download', 'Installing Git');
    toast.updateProgress('git-download', 'downloading', 5, 'Downloading Portable Git (~50MB)...');
    
    try {
      const result = await window.electronAPI.tools.download('git');
      if (result.success) {
        toast.updateProgress('git-download', 'extracting', 70, 'Extracting Git files...');
        // Small delay to show extracting state
        await new Promise(r => setTimeout(r, 500));
        toast.completeProgress('git-download', true, 'Git installed successfully!');
        await checkGitInstallation();
      } else {
        toast.completeProgress('git-download', false, result.error || 'Download failed');
        setGitDownloadError(result.error || 'Download failed');
      }
    } catch (err: any) {
      toast.completeProgress('git-download', false, err.message);
      setGitDownloadError(err.message);
    } finally {
      setIsDownloadingGit(false);
    }
  };
        ...
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-6">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h3 className="text-amber-400 font-semibold text-lg mb-2">Git CLI Not Found</h3>
              <p className="text-muted-foreground mb-4">
                Git is required for cloning and managing repositories.
                Git was not found in your system PATH.
              </p>
              ...
              <p className="text-xs text-muted-foreground mt-3">
                Auto-Install downloads Portable Git that works with this app only.
              </p>
            </div>
          </div>
        </div>
